...
控制台以**线上路由层的前端**为蓝本（`bestruirui/octopus@e7a1455` 的 `web/`）：Vite + React 19 + Radix UI + Tailwind 4 + TanStack Query/Virtual + zustand + use-intl（三语）+ Recharts + `@hello-pangea/dnd`。目录为 `api/`（每资源一文件）、`components/{ui,common,modules}`、`hooks/`、`locales/`、`provider/`、`stores/`。

两条值得直接继承的机制：
- **模块划分**：`group`（车道=分组）、`channel`、`model`、`log`、`apikey-dashboard`、`home`、`setting`、`login` —— 与 PBR 的管理面几乎一一对应。
- **运行态可视化**：`group` 模块用 SSE 推送进程内路由状态（当前成员/探测占用/冷却表），配合 30s 轮询兜底；成员顺序用拖拽写入 priority。

→ PBR 控制台在此基础上"迁移功能原理、换品牌、换 API、按车道模型适配"，而非照搬 new-api 的巨型控制台。
...
1. **认证**：管理面无账号体系，只有**一个登录口令**；管理密钥 = `Base64(SHA256(登录口令))`，首启时设置口令（详见 [`token-spec-v1.md`](token-spec-v1.md) §2）。模型面用客户端密钥。**模型面与管理面同端口全部监听 `0.0.0.0` 对局域网开放**（§18），凭凭据鉴权，不做来源限制；口令必须是长随机串。
...
- **蓝本 = 线上路由层的前端**（Vite + React 19 + Radix + Tailwind 4 + TanStack Query/Virtual + zustand + use-intl + Recharts + dnd）：**迁移功能原理，换品牌、换 API、按车道模型适配**。
- **认证极简**：无账号，只有登录口令；首启设置口令，之后登录取得派生管理密钥存本地作 `Bearer`。无注册/找回/OAuth/passkey/2FA。
- **页面集合**：Dashboard、车道（成员可拖拽排序 + 实时冷却/熔断徽标）、渠道、模型路由、请求日志（attempts 链）、令牌面板、设置（账户/API 密钥/外观/备份/信息/单价/日志/系统）、Playground、错误页。
- **实时机制**：车道运行态经 SSE 推送 + 30s 轮询兜底（照蓝本）。
- **产物形态**：Vite 构建产物 embed 进二进制，单进程同时服务 `/v1/*`、`/api/v1/*` 与静态控制台。
...
- `web/**`：以线上路由层前端为蓝本迁移（见 §2.8 / ui-spec-v1.md），全量替换品牌；不迁移计费/多用户页面。
...
2. **不迁**：`web/`（前端按 ui-spec 以另一上游为蓝本新建）、`electron/`、`docs/`、`e2e/` 等非代码资产。
3. **§10.4 的目标布局是演进终点，不是起点**：新写的路由核心放 `internal/`；旧包逐步改造或删除，不要求一次性重排目录。
4. **计费与多用户先惰性化**（§10.2），W7 才物理清除；中间阶段允许 `service`/`setting` 包继续存在。
5. **许可证**：保留 new-api 的 AGPL 头、`LICENSE`、`NOTICE`、`THIRD-PARTY-LICENSES.md`；前端另附蓝本上游许可清单。品牌可替换，版权不可替换。
...
| UI 蓝本 | **线上路由层前端**，迁移功能原理（ui-spec-v1.md） |
