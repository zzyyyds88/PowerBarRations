...
- 此决定的风险由业主显式承接：数据库文件泄露 ⇒ 客户端密钥全部暴露（可冒用打流量产生上游费用，**不能**进入管理面）。缓解仍是 §2.5 的库文件 0600 权限与受控网络。迁移前创建的存量密钥没有明文可回显（`key` 返回 `null`），提示轮换一次即可获得。
- 为兼容下游，**旧的 `sk-` 前缀密钥不做格式拦截**，导入/录入时按原值存明文并同样存哈希。
...
| 列表/详情 | `GET /api/keys` `/{name}` | 含 `key` 明文（迁移前存量密钥为 `null`）与 `key_prefix` |
