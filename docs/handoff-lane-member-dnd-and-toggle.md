...
> 不需要再读一遍代码库即可开工。
> **基线**：`main` @ `412c4dd`（工作区干净）。文中行号均以该提交为准，若已漂移请用函数名/符号名定位。
...
| 提交 | 内容 |
|---|---|
| `b332e68` | 价格输入框保留小数点草稿（受控归一化吃掉 `0.`，打不出 `0.1`） |
| `813b9b3` | `GET /routes` 有显式改名时无条件回传 `upstream_override`（**根因**：条件恒假 → 控制台读不到 → 保存清空全部成员上游真名 → 车道必 404/503） |
| `4ffe76e` | 车道编排器成员行内联"未声明模型"告警（防车道名回落） |
| `14e49f1` / `2435d31` | 使用日志用户列按设计文档回退 + 列守卫测试对齐 |
| `abf83ef` / `08de619` / `b7adda3` | 使用日志 i18n 缺键补全、SelectValue i18n 守卫、过时测试对齐 |
