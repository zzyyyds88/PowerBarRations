...
| §11 迁移机制 | §11 | 迁移机制已随产品范围收敛整体移除，现行边界见 design-v1 §11「开发阶段数据边界」；本节仅存历史 |
...
## 11. 迁移机制（归档；该机制已整体废弃，不再实施）

> 本节只记录立项期的机制设计，**不构成当前实现要求**：旧 `new-api`/`octopus` 数据库迁移与
> `pbr migrate` 已随产品范围收敛删除，现行数据边界见 [`design-v1.md`](design-v1.md) §11。
...
> 注：隐式链相关表述已被 [ADR 0005](../adr/0005-lane-required-and-channel-model-mapping.md) 推翻；上述迁移算法随机制一并废弃，仅作历史留档。
...
- **H 回滚演练**：按当时的 `MIGRATION.md`（已随迁移机制删除）在测试实例上演练切流与回滚（不动生产）。
- **I 文档一致性**：README / ADR / OpenAPI / `verify/` 与实现一致，无"文档说 A、实现是 B"的漂移。
