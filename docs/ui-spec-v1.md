...
- **单价**：渠道级上游单价在渠道编辑页"上游单价"配置；全局默认单价在"系统设置 → 模型 → 上游单价表"。
...
- **行内操作**（不再有独立的「路由与故障切换」侧边栏入口）：
  - 「路由与故障切换」→ 打开该模型的成员链抽屉（`GET /api/routes/{model}`）；
  - 「资料」→ 模型描述、标签等元数据；
...
- **上游单价的落点**：独立「上游单价」页签。**计价键是请求模型名（路由键/车道名）**，不是上游真名；表格默认跟随渠道模型清单，**并支持手动添加清单外的自定义计价行**（如车道成员引用的上游名、独立车道名），自定义行可删除。只有 input/output/cache_read/cache_write 中至少填了一项的模型才写入 `pbr_prices`；全空模型不落库，避免全 0 条目命中后覆盖全局默认价。系统设置 → 模型 → 「上游单价表」只是**未配渠道价时的兜底默认**；折算优先级为 **渠道价 > 全局默认 > 不折算**。
...
- **模型清单（自动探测）**：探测入口**只在渠道编辑器的模型区**——连接信息（`base_url`/`key`/`type`）填好后**自动**调用上游 `/models` 拉取候选并提示合并（"已从上游发现 N 个 · 新增 M · 已存在 K"），附「重新拉取」「全部加入 / 仅加入新增」；未保存的渠道用预览请求探测，已保存渠道用 `POST /api/channels/{name}/sync-models`。**渠道列表行不提供「Fetch Models」入口**。探测失败只给内联可重试提示，不阻断保存，**不覆盖用户手填的模型**。
...
| 单价 | 全局默认上游单价表（记账，非计费）；渠道级上游单价在渠道编辑页"上游单价"配置 |
| 日志 | 日志保留天数、手动清理 |
