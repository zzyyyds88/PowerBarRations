...
- **成员编排器（新建车道 / 编辑成员链共用，居中大弹窗，两栏布局）**：
  ...
  - **右栏「成员（顺序）」**：已选成员的有序列表（顺序即故障转移顺序，首位优先）；支持**拖拽排序**或上移/下移、删除、编辑上游真名（清空 = 用渠道映射，占位提示"用渠道映射"）、**成员启停开关**（见下）、「清空」。列表为空时显示空态引导。
    - **拖拽排序（原生 HTML5 DnD，禁止引入任何 dnd 依赖）**：成员行本身可拖，交互规格固定到可验收粒度——
      ...
      - **行标识（React key）稳定性**：拖拽身份与 key 一律用草稿行的稳定自增 `id`（现 `nextMemberId()`），**禁止把 `channel` / `upstream_model` 拼进 key**——改名会重挂载整行、输入框失焦（代码注释里已记录的实测 bug，成员行内联改名同样受影响）。
  - **成员启停开关（人工停用，可逆）**：每个成员行提供一个开关控件，语义 = api-spec §4.2 的成员级 `enabled`。
    ...
    - **保存必须带回**：`enabled` 与 `overrides`、`upstream_override`、`public_alias` 一样，属于"读草稿 → 保存原样带回"的成员字段（api-spec §4.2 通则）；漏带回即被整体替换语义清空。
  ...
  - **成员唯一键 = `(渠道, 上游真名)`**：同一渠道可在一条车道内出现多次（各自对应不同上游模型）；按该键去重，不再按渠道去重。
  - **默认上游真名**：从"渠道 X → 模型 m"加入时，成员 `upstream_model` **默认填「按渠道 `model_mapping` 解析 m」的结果**（有映射取映射右值，无映射取 m 本身）；**查表键是成员所选模型 m，与车道名无关**（池化车道 `111` 里选 `glm-5.3-flash` 也命中映射）。用户清空该字段则回退为"渠道 `model_mapping` > 路由键"。
  - **未声明模型告警**：成员的最终上游真名（成员显式改名 > 渠道 `model_mapping` > 路由键）若既不在该渠道的 `models` 清单、也不是 `model_mapping` 的右值，成员行内联一条琥珀色提示（"渠道未声明该模型，保存前请确认上游支持"），**不拦截保存**（自由覆盖是合法能力，仅防呆）。该提示必须在编辑既有车道时同样生效——成员显式上游真名经 `GET /api/routes/{model}` 的 `upstream_override` 回传后必须完整回显到编辑器（api-spec §5.7），避免"打开编辑再保存"把成员真名静默清空。
  - **路由键无需任何渠道声明**：候选为空时仍可保存（只要已选成员非空）；提示文案为"可自定义路由键，再从左侧任意渠道挑选成员"。
  - 保存即写入 PBR 车道（名称 = 路由键）；`PUT /api/lanes/{name}`，成员载荷固定为 `{channel, upstream_model, public_alias, priority, enabled, overrides}`——**除 `enabled` 外的成员级字段必须从草稿原样带回**，因为成员写入是整体替换，漏带回即清空（api-spec §4.2 通则）；顺序即数组顺序，保存时按位置生成 `priority`（首位最大，现实现为 `priority = 成员数 - 序号`），界面**不暴露 priority 数字输入**，也不暴露 `weighted` 权重（该模式已删除）。
  - **模式可选 `failover` / `manual`**：选 `manual` 时必须指定「当前成员」（`active_member`，取值优先成员别名、否则 `channel/upstream_model` 标签），未指定则保存被拦截；模式与 active member 的改动同样走草稿/放弃确认。**`active_member` 指向一个被人工停用的成员时，该行内联醒目告警**（"当前成员已停用，该模型现在不可调用"）但不拦截保存——manual 不换人（api-spec §4.2），用户必须自己看见这个组合。
