...
- **验收**：模型页列为 **模型 / 匹配类型 / 命中模型数 / 描述 / 标签 / 操作**，无「渠道与分组」「供应商」「可用分组」「计费类型」；未配图标的已知厂商模型（如 `glm-5.2`、`deepseek-v4-flash`、`bge-m3`）显示推断图标，未知模型回退首字符；编辑模型打开居中弹窗且其中有四档「匹配类型」下拉。**自动匹配可用**：把某条目录记录的匹配类型改为「前缀」（如 `qwen3-`）并保存后，该行的命中模型数等于渠道声明模型中被该前缀命中的个数，`GET /api/model-metadata` 仍不返回 `matched_count`。为"模型1"添加"上游1 → 上游2"后，`GET /api/routes/模型1` 顺序一致；上游1 故障时请求逃逸到上游2；添加/删除/排序后的保存结果与后端 priority 降序一致；**未配车道的模型请求返回 503**，配好后立即可用。
