...
- 沿用上游 `channels`：卡片列表 + 表单（协议类型、**API 地址**、key（只写不读）、模型清单、**模型映射**、**上游单价**、参数覆盖 JSON、代理、启用）+ 探活 + 批量操作 + 标签。**渠道表单没有优先级与权重**（两字段已随路由收敛物理删除），**也没有分组**（列表、卡片、移动端、表单、筛选一律不出现；`group` 仅由提交适配层固定注入 `default` 以兼容基座结构，不在界面建模）。
