...
| `models` | **保留 + 改造** | 模型管理：模型目录（元数据 + 按名推断图标），见 §6.3；成员链在独立的「路由与故障切换」页维护 |
...
- **图标按模型名推断**：列表与详情渲染图标时，取值顺序为 **显式配置的 `icon` > 按模型名推断（`resolveModelProvider(model_name).icon`）> 模型名首字符兜底**。推断是**只读展示行为**，不写库；编辑弹窗展示"识别到的图标"并提供一键采用。
...
- **API 地址只填到版本段即可（全部协议型渠道）**：base_url 允许带版本段——OpenAI/Anthropic 填到 `https://host/v1`，Gemini 填到 `https://host/v1beta`；**网关会先剥掉结尾的版本段，再按协议补全后续路径**。填完整端点（不带版本段）同样可用；旧数据（不含版本段）不受影响。Custom（type 8）继续支持 `{model}` 变量与完整端点 URL。
