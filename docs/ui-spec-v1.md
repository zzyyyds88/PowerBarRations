...
- **成员编排器（新建车道 / 编辑成员链共用，居中大弹窗，两栏布局）**：
  ...
  - **默认上游真名**：从"渠道 X → 模型 m"加入时，成员 `upstream_model` **默认显式填 m**；用户清空该字段则回退为"渠道 `model_mapping` > 路由键"。
...
- **模型清单（手动探测 + 居中选择弹窗）**：
  ...
  - 探测入口**只在渠道编辑器的模型区**：模型区提供**「获取模型列表」按钮**（New API `FetchModelsDialog` 形态）。点击后**打开居中弹窗**（§6.9，`size=lg/xl`），**弹窗打开即自动拉取**上游 `/models`（无需再点一次）：拉取中显示 spinner；成功后候选以勾选列表呈现（"已从上游发现 N 个 · 新增 M · 已存在 K"），**按需勾选**，另有「全部加入 / 仅加入新增」；未保存渠道走预览探测、已保存渠道按渠道名探测。点「应用 / 保存模型」才回填模型清单，取消/关闭不写入。探测失败在弹窗内联提示并可重试，不覆盖用户手填的模型；探测后连接信息再变更会标记过期并提示重新拉取。
