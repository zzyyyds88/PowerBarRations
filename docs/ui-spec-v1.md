...
- **模型清单（手动探测 + 居中选择弹窗）**：
  ...
  - **已保存渠道复用服务端已存密钥**（`GET /api/channel/fetch_models/{id}`），未保存渠道用草稿连接信息走预览探测（`POST /api/channel/fetch_models`）；**不得**在已保存渠道上发"只有 base_url、没有 key"的草稿探测（表单出于安全不回填 key，那样必然 401）。
