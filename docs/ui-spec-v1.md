...
- 车道运行态（当前成员 / 探测占用 / 亲和截止 / 各成员冷却）通过 **SSE** `GET /api/route-events` 推送（建连即推全量快照，之后每 ~1s 一帧），合并进前端缓存，**不写回**持久配置。
...
- **「路由与故障切换」是侧边栏独立页**（`/routes`），整体为 **octopus 式卡片网格**（对齐线上章鱼分组页）：
  - **卡片视图**：每张卡片代表一条车道（路由键），卡面展示路由键、状态徽章（可调用 / 全部成员不可用 / 车道停用）、成员顺序摘要（成员渠道停用标灰）与运行态（当前成员 / 冷却 / 熔断）。卡片操作：编辑成员链、删除车道。
  ...
  - 数据源：`GET /api/models`（路由键与状态，前端只保留 `explicit`/`disabled`）+ `GET /api/lane-summaries`（成员顺序）+ SSE/轮询运行态（§4）。
- **成员编排器（新建车道 / 编辑成员链共用，居中大弹窗，两栏布局）**：
  ...
  - **右栏「成员（顺序）」**：已选成员的有序列表（顺序即故障转移顺序，首位优先）；支持**拖拽排序**或上移/下移、删除、编辑上游真名（清空 = 用渠道映射，占位提示"用渠道映射"）、「清空」。列表为空时显示空态引导。
  ...
  - **未声明模型告警**：成员的最终上游真名（成员显式改名 > 渠道 `model_mapping` > 路由键）若既不在该渠道的 `models` 清单、也不是 `model_mapping` 的右值，成员行内联一条琥珀色提示（"渠道未声明该模型，保存前请确认上游支持"），**不拦截保存**（自由覆盖是合法能力，仅防呆）。该提示必须在编辑既有车道时同样生效——成员显式上游真名经 `GET /api/routes/{model}` 的 `upstream_override` 回传后必须完整回显到编辑器（api-spec §4.4），避免"打开编辑再保存"把成员真名静默清空。
  - **路由键无需任何渠道声明**：候选为空时仍可保存（只要已选成员非空）；提示文案为"可自定义路由键，再从左侧任意渠道挑选成员"。
  - 保存即写入 PBR 车道（名称 = 路由键）；`PUT /api/lanes/{name}`，成员 `{channel, upstream_model, priority}`；顺序即数组顺序，保存时按位置生成 `priority`（首位最大），界面**不暴露 priority 数字输入**，也不暴露 `weighted` 权重（该模式已删除）。
  - **模式可选 `failover` / `manual`**：选 `manual` 时必须指定「当前成员」（`active_member`，取值优先成员别名、否则 `channel/upstream_model` 标签），未指定则保存被拦截；模式与 active member 的改动同样走草稿/放弃确认。
...
- **统一日志视图采用上游的详细列形态**（New API `common-logs-columns` + PBR 车道列）：时间、类型（消耗/错误徽章）、渠道、**用户（仅管理员可见，self view 隐藏，与渠道列同策略；用户明确要求按 new-api 形态恢复）**、令牌、模型、车道/成员、流、提示/补全 token、花费、耗时（用时+首字）、详情。列布局紧凑（小字号、窄 padding）。**全部列经 `meta.contentSized` 贴内容宽**（colgroup `1%` + `tableLayout: auto` 均匀分摊剩余空间），消除按比例分宽造成的列间大段空白。
