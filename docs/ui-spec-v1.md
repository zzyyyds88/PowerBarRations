...
- **「路由与故障切换」是侧边栏独立页**（`/routes`），整体为 **octopus 式卡片网格**（对齐线上章鱼分组页）：
  - **卡片视图**：每张卡片代表一条车道（路由键），卡面展示路由键、状态徽章（可调用 / 全部成员不可用 / 车道停用 / 未配车道不可调用）、成员顺序摘要（成员渠道停用标灰）与运行态（当前成员 / 冷却 / 熔断）。卡片操作：编辑成员链、删除车道。
  - **未配车道但仍被渠道声明的路由键**（`source: unconfigured`）也以卡片出现，提示"未固化不可调用"，点开即可组链。
  - 页头提供「新建车道」入口；另有「清理悬空成员」提示条（存在渠道已删的成员时）。
  - 数据源：`GET /api/models`（路由键与状态）+ `GET /api/lane-summaries`（成员顺序）+ SSE/轮询运行态（§4）。
