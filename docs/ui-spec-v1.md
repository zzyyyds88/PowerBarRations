...
| 上手三步 | 建渠道（`base_url` + key + 模型清单 + `model_mapping`）→ 在路由页手动新建车道并排定成员 → 客户端密钥调 `/v1/*` 并看 attempts 链与成本 |
| CTA | 部署单二进制并接入上游渠道 |
...
- **「路由与故障切换」是侧边栏独立页**（`/routes`）：集中列出全部路由键（已配车道 + 未配车道），每行展示模型名、状态（explicit 可调用 / unconfigured 不可调用）、成员数与顺序摘要（成员渠道停用标灰）；行内「编辑成员链」打开该模型的成员链弹窗（`GET /api/routes/{model}`，含 `candidates` 与每名成员的 `channel_enabled`）。
- **成员链弹窗（手工管理，居中弹窗）**：
  - 成员列表来源：**已配车道 → 车道成员**；未配车道 → 空列表（提示"添加成员后保存即固化车道"）。渠道声明只作为**候选**：未配车道时作为 `members` 返回，已配车道时作为 `candidates` 返回（两者都按渠道 id 升序），由用户点「添加成员」显式加入，**不自动填入**。
  ...
  - 保存即写入 PBR 车道（名称 = 模型名）；`PUT /api/lanes/{model}`，成员 `{channel, upstream_model, priority}`。
...
- **只手动手动增删成员与删除车道（产品要求，路由只支持手动维护）**：
  - 「编辑成员链」手动添加成员（候选含"已配车道后新声明该模型"的渠道）、删除成员、上移/下移排序、编辑上游真名；保存即 `PUT /api/lanes/{model}` 固化车道。
  - 「删除车道」必须走 destructive 确认框，写清删除后该模型立即变为不可调用；无车道（`!routable`）时禁用该按钮。
  - 需要为"没有任何渠道声明"的自定义路由键建车道时，用页面的「新建车道」入口（写入 `PUT /api/lanes/{name}`）：输入路由键 → 从候选渠道逐个添加 → **可上移/下移排序、可编辑上游真名** → 保存（顺序即优先级）。
  - 切换模型或关闭弹窗时若有未保存草稿，先确认放弃（与模型编辑弹窗同一模式）。
- **验收**：模型页列为 **模型 / 匹配类型 / 已绑定渠道 / 描述 / 标签 / 操作**，无「渠道与分组」「供应商」「可用分组」「计费类型」；未配图标的已知厂商模型（如 `glm-5.2`、`deepseek-v4-flash`、`bge-m3`）显示推断图标，未知模型回退首字符；编辑模型打开居中弹窗且其中有四档「匹配类型」下拉。**自动匹配可用**：把某条目录记录的匹配类型改为「前缀」（如 `qwen3-`）并保存后，该行模型列内联的命中数等于渠道声明模型中被该前缀命中的个数，`GET /api/model-metadata` 仍不返回 `matched_count`。为"模型1"添加"上游1 → 上游2"后，`GET /api/routes/模型1` 顺序一致；上游1 故障时请求逃逸到上游2；添加/删除/排序后的保存结果与后端 priority 降序一致；**未配车道的模型请求返回 503**，配好后立即可用。
