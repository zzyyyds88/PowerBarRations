...
- **创建/编辑走居中弹窗**（`ChannelMutateDialog`）：打开即表单可编辑，**没有厂商市场选择器、没有两步向导**。协议类型在「基本信息」分区用**可搜索的类型下拉**选择（内置类型 + 任务插件项）；选中插件自动回填名称/模型/地址；创建模式下名称为空时自动填类型名。Escape/Cancel 直接关闭弹窗并丢弃未保存草稿。
