...
- **上游单价的落点**：「连接与模型」分区**右栏、紧跟模型清单**——每个模型在不同渠道的采购价不同，与模型清单同屏最直观。系统设置 → 模型 → 「上游单价表」只是**未配渠道价时的兜底默认**；折算优先级为 **渠道价 > 全局默认 > 不折算**。
...
- **模型清单（自动探测）**：连接信息（`base_url`/`key`/`type`）填好后**自动**调用上游 `/models` 拉取候选并提示合并（"已从上游发现 N 个 · 新增 M · 已存在 K"），附「重新拉取」「全部加入 / 仅加入新增」；未保存的渠道用预览请求探测，已保存渠道用 `POST /api/channels/{name}/sync-models`。探测失败只给内联可重试提示，不阻断保存，**不覆盖用户手填的模型**。
