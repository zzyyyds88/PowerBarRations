...
> 网关按车道顺序做故障转移。没有车道的模型一律 `503`，与"上游全挂"同形；**声明模型绝不自动建车道**。
> 车道成员可任选**任意渠道的任意模型**（池化，无需同名，同一渠道可多次出现），并支持自定义顺序、
> 成员改名与两种模式（`failover` 默认 / `manual`）。
>
...
# 3) 固化车道：渠道声明只是目录，必须把成员链固化成车道才可调用
# 在「路由与故障切换」页点「新建车道」手动建（可自定义路由键，成员可任选任意渠道的任意模型），
# 或用 PUT 固化单个模型：
...
- **车道是唯一路由入口**（[ADR 0005](docs/adr/0005-lane-required-and-channel-model-mapping.md)）：
  渠道声明 `models` 只是"该渠道能提供什么"的目录，**不等于可调用**；没有同名启用车道时请求该模型
  一律 `503 No available channel for model <X>`。**声明/新增模型绝不自动建车道**。
- 手动新建：在「路由与故障切换」页点「新建车道」，输入路由键并从左侧「渠道 → 模型」选择器挑选成员；也可按 §2.2 用 `PUT /api/v1/lanes/{model}` 建/改。
- **成员自由编排**（[ADR 0006](docs/adr/0006-lane-free-member-composition.md)）：成员可来自**任意渠道的任意模型**，无需同名，**同一渠道可在一条车道里出现多次**（去重键 = `(渠道, 上游真名)`）。路由键可任意命名，不要求任何渠道声明过它。
- **渠道没有优先级/权重**（已物理删除）：路由顺序只由车道的成员顺序决定，在侧边栏独立页「路由与故障切换」（`/routes`）里用拖拽或上移/下移人工排定。
- 上游命名与路由键不一致时，在**渠道**上配 `model_mapping`（路由键 → 上游真名）。
  在路由页从「渠道 × 模型 m」加入成员时，控制台会**以 m 为键**解析该映射并写入成员上游真名
  （所以池化车道——车道名 ≠ m——也能命中）；成员级 `upstream_model` 可再手动覆盖。
  运行期回落仍以**路由键（车道名）**查表：成员上游留空且车道名等于映射键时才走映射。
...
需要自定义顺序、成员改名、或池化不同上游的不同模型名时就写车道（成员可任选任意渠道的任意模型，无需同名，同一渠道可出现多次）：
