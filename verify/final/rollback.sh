...
curl -s "${A[@]}" -X PUT -d '{"type":"openai","base_url":"http://127.0.0.1:'"$OLD_PORT"'","key":"'"$GOOD_KEY"'","priority":10,"models":["migrated-model"],"enabled":true}' "$NEW/api/v1/channels/legacy-vendor" > /dev/null
