...
| 时间 | 结果 | 证据 |
|---|---|---|
| 2026-09-19 | **PASS=26 FAIL=0**（连续 4 次可复现） | `verify/e2e-ui/run-20260919-114703.log` |

## 已知产品缺口（非脚本问题）

- 试打台未展示 `X-Served-By`（ui-spec §6.8 / `docs/test-spec-v1.md` §4 行 6 要求）。
  该头在 playground 下从未实现（流式走 SSE 读不到响应头，非流式 fetch 也未带回消息模型）。
  脚本对回复做 PASS 断言，对该头只写一条非计分 `NOTE`，不以放宽断言掩盖。
