...
*/
// FIPS 180-4 SHA-256（纯 JS），仅作为 Web Crypto 不可用时的兜底：
// 非安全上下文（HTTP 且非 localhost）下 crypto.subtle 被浏览器禁用，
// 而管理密钥派生（token-spec §2.1）在初始化页必须仍然工作。
// 安全上下文一律优先走 crypto.subtle。
...
export function sha256Bytes(message: Uint8Array): Uint8Array {
  ...
  const totalLen = (Math.floor((message.length + 8) / 64) + 1) * 64
